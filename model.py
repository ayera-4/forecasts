...
class GarchModel:
    ...
    def wrangle_data(self, n_observations):
        ...
        if self.use_new_data:
            ...
            api = AlphaVantageAPI()
            #get data
            new_df = api.get_daily(ticker=self.ticker)
           
            self.repo.insert_table(table_name=self.ticker, records=new_df, if_exists="replace")
    ...
    def fit(self, p, q):
        ...
        #Split data
        #cutoff_test = int(len(y_data) * 0.8)
        #train_data = y_data[:cutoff_test]
    ...
    def __clean_prediction(self, prediction):
        ...
        predictions_formatted = pd.Series(data, index=prediction_index)
    ...
    def load(self):
        ...
        try:
            ...
            raise Exception(f"No model of {self.ticker}.pkl")
